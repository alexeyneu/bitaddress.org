...
ninja.privateKey = {
	isPrivateKey: function (key) {
		return (
					Goldencoin.ECKey.isWalletImportFormat(key) ||
					Goldencoin.ECKey.isCompressedWalletImportFormat(key) ||
					Goldencoin.ECKey.isHexFormat(key) ||
					Goldencoin.ECKey.isBase64Format(key) ||
					Goldencoin.ECKey.isMiniFormat(key)
				);
	},
	getECKeyFromAdding: function (privKey1, privKey2) {
		var n = EllipticCurve.getSECCurveByName("secp256k1").getN();
		var ecKey1 = new Goldencoin.ECKey(privKey1);
		var ecKey2 = new Goldencoin.ECKey(privKey2);
		// if both keys are the same return null
		if (ecKey1.getGoldencoinHexFormat() == ecKey2.getGoldencoinHexFormat()) return null;
		if (ecKey1 == null || ecKey2 == null) return null;
		var combinedPrivateKey = new Goldencoin.ECKey(ecKey1.priv.add(ecKey2.priv).mod(n));
	...
	getECKeyFromMultiplying: function (privKey1, privKey2) {
		...
		if (ecKey1.getGoldencoinHexFormat() == ecKey2.getGoldencoinHexFormat()) return null;
		if (ecKey1 == null || ecKey2 == null) return null;
		var combinedPrivateKey = new Goldencoin.ECKey(ecKey1.priv.multiply(ecKey2.priv).mod(n));
		// compressed when both keys are compressed
		if (ecKey1.compressed && ecKey2.compressed) combinedPrivateKey.setCompressed(true);
		return combinedPrivateKey;
	},
	...
	BIP38EncryptedKeyToByteArrayAsync: function (base58Encrypted, passphrase, callback) {
		var hex;
		try {
			hex = Goldencoin.Base58.decode(base58Encrypted);
		} catch (e) {
		...
		hex = hex.slice(0, -4);
		var checksum = Goldencoin.Util.dsha256(hex);
		...
		var verifyHashAndReturn = function () {
			var tmpkey = new Goldencoin.ECKey(decrypted); // decrypted using closure
			var base58AddrText = tmpkey.setCompressed(isCompPoint).getGoldencoinAddress(); // isCompPoint using closure
			checksum = Goldencoin.Util.dsha256(base58AddrText); // checksum using closure

			if (checksum[0] != hex[3] || checksum[1] != hex[4] || checksum[2] != hex[5] || checksum[3] != hex[6]) {
				callback(new Error(ninja.translator.get("bip38alertincorrectpassphrase"))); // callback using closure
				return;
			}
			callback(tmpkey.getGoldencoinPrivateKeyByteArray()); // callback using closure
		};
		...
		if (!isECMult) {
			...
			Crypto_scrypt(passphrase, ownersalt, 16384, 8, 8, 32, function (prefactorA) {
				...
				Crypto_scrypt(passpoint, addresshashplusownerentropy, 1024, 1, 1, 64, function (derived) {
					...
					var factorb = Goldencoin.Util.dsha256(seedb);
	...
	BIP38PrivateKeyToEncryptedKeyAsync: function (base58Key, passphrase, compressed, callback) {
		var privKey = new Goldencoin.ECKey(base58Key);
		var privKeyBytes = privKey.getGoldencoinPrivateKeyByteArray();
		var address = privKey.setCompressed(compressed).getGoldencoinAddress();

		// compute sha256(sha256(address)) and take first 4 bytes
		var salt = Goldencoin.Util.dsha256(address).slice(0, 4);
		...
		Crypto_scrypt(passphrase, salt, 16384, 8, 8, 64, function (derivedBytes) {
			...
			encryptedKey = encryptedKey.concat(Goldencoin.Util.dsha256(encryptedKey).slice(0, 4));
			callback(Goldencoin.Base58.encode(encryptedKey));
		});
	},
	BIP38GenerateIntermediatePointAsync: function (passphrase, lotNum, sequenceNum, callback) {
		...
		Crypto_scrypt(passphrase, ownerSalt, 16384, 8, 8, 32, function (prefactor) {
			...
			var passfactorBytes = noNumbers ? prefactor : Goldencoin.Util.dsha256(prefactor.concat(ownerEntropy));
			...
			// base58check encode
			intermediate = intermediate.concat(Goldencoin.Util.dsha256(intermediate).slice(0, 4));
			callback(Goldencoin.Base58.encode(intermediate));
		});
	},
	BIP38GenerateECAddressAsync: function (intermediate, compressed, callback) {
		// decode IPS
		var x = Goldencoin.Base58.decode(intermediate);
		//if(x.slice(49, 4) !== Goldencoin.Util.dsha256(x.slice(0,49)).slice(0,4)) {
		...
		// 3) ECMultiply passpoint by factorb. Use the resulting EC point as a public key and hash it into a Goldencoin
		// address using either compressed or uncompressed public key methodology (specify which methodology is used
		// inside flagbyte). This is the generated Goldencoin address, call it generatedaddress.
		...
		var generatedAddress = (new Goldencoin.Address(Goldencoin.Util.sha256ripe160(generatedBytes))).toString();

		// 4) Take the first four bytes of SHA256(SHA256(generatedaddress)) and call it addresshash.
		var addressHash = Goldencoin.Util.dsha256(generatedAddress).slice(0, 4);
		...
		Crypto_scrypt(passpoint, addressHash.concat(ownerEntropy), 1024, 1, 1, 64, function (derivedBytes) {
			...
			// base58check encode
			encryptedKey = encryptedKey.concat(Goldencoin.Util.dsha256(encryptedKey).slice(0, 4));
			callback(generatedAddress, Goldencoin.Base58.encode(encryptedKey));
		});
	}
};
...
ninja.publicKey = {
	...
	getGoldencoinAddressFromByteArray: function (pubKeyByteArray) {
		var pubKeyHash = Goldencoin.Util.sha256ripe160(pubKeyByteArray);
		var addr = new Goldencoin.Address(pubKeyHash);
