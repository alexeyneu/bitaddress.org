ninja.wallets.splitwallet = {
	...
	splitKey: function () {
		try {
			...
			var shares = ninja.wallets.splitwallet.getFormattedShares(key.getGoldencoinHexFormat(), numshares, threshold);

			var output = document.createElement("div");
			output.setAttribute("id", "splitoutput");
			var m = {};
			output.appendChild(this.mkOutputRow(bitcoinAddress, "split_addr", "Goldencoin Address:    "));
	...
	combineShares: function () {
		try {
			...
			var privkeyBase58 = new Goldencoin.ECKey(combinedBytes).getGoldencoinWalletImportFormat();
	...
	getFormattedShares: function (key, numshares, threshold) {
		var shares = secrets.share(key, numshares, threshold).map(ninja.wallets.splitwallet.hexToBytes).map(Goldencoin.Base58.encode);
		return shares;
	},

	// combine base58 formatted shares and return a bitcoin byte array
	combineFormattedShares: function (shares) {
		var combined = secrets.combine(shares.map(Goldencoin.Base58.decode).map(Crypto.util.bytesToHex).map(ninja.wallets.splitwallet.stripLeadZeros));
